...
主要类：
    ...
    - AASConfig: 配置管理
    - MySQLAASCalculator: AAS计算核心（线程安全、高性能）
...
使用示例：
    from dbskiter.shared.mysql_aas_calculator_v2 import MySQLAASCalculator, AASConfig
...
def retry_on_error(
    max_retries: int = 3, delay: float = 1.0, backoff: float = 2.0, exceptions: Tuple[type, ...] = (Exception,)
):
    """
    重试装饰器 - 指数退避策略
...
class AASMetrics:
    ...
    def __post_init__(self):
        ...
        if category_sum > self.total * 1.01:  # 允许1%误差
            raise ValueError(f"分类之和 ({category_sum:.2f}) 不能超过 total ({self.total:.2f})")
...
class AASPersistentStorage:
    ...
    def __init__(self, storage_path: str = "./runtime_data/aas", max_size_mb: int = 100, max_age_hours: int = 168):
        """
        初始化存储
    ...
    def get_history(
        self, start_time: Optional[datetime] = None, end_time: Optional[datetime] = None, limit: Optional[int] = None
    ) -> List[AASMetrics]:
        """
        获取历史数据
    ...
    def cleanup_old_data(self) -> int:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute("DELETE FROM aas_metrics WHERE timestamp < ?", (cutoff_time.isoformat(),))
    ...
    def get_storage_stats(self) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # 时间范围
            cursor = conn.execute("SELECT MIN(timestamp), MAX(timestamp) FROM aas_metrics")
            min_ts, max_ts = cursor.fetchone()

            # 最近24小时记录数
            day_ago = (datetime.now() - timedelta(days=1)).isoformat()
            cursor = conn.execute("SELECT COUNT(*) FROM aas_metrics WHERE timestamp > ?", (day_ago,))
        ...
        return {
            ...
            "time_range": {"earliest": min_ts, "latest": max_ts},
...
class MySQLAASCalculator:
    ...
    def __init__(self, connector, config: Optional[AASConfig] = None):
        """
        初始化AAS计算器
        ...
        示例:
            ...
            >>> calculator = MySQLAASCalculator(connector, config)
        """
        ...
        # 查询缓存
        self._cache = AASQueryCache(ttl=self.config.cache_ttl, max_size=self.config.max_cache_size)
    ...
    def _start_cleanup_task(self):
        ...
        def cleanup_worker():
            try:
                ...
                # 重新调度
                self._cleanup_timer = threading.Timer(self.config.cleanup_interval, cleanup_worker)
    ...
    def _check_prerequisites(self):
        ...
        try:
            ...
            result = self._execute_with_retry("SHOW VARIABLES LIKE 'performance_schema'")
            if result and result.rows and result.rows[0][1] != "ON":
                logger.warning(
                    "performance_schema未启用，AAS计算可能不准确。" "建议执行: SET GLOBAL performance_schema = ON;"
                )
    ...
    def _get_vcpu_count(self) -> int:
        ...
        with self._vcpu_lock:
            ...
            # 方法2: 从MySQL配置变量估算
            try:
                result = self._execute_with_retry("SHOW VARIABLES LIKE 'innodb_thread_concurrency'")
    ...
    def calculate_current_aas(self) -> AASMetrics:
        ...
        try:
            ...
            metrics = AASMetrics(
                ...
                metadata={"collection_method": "performance_schema.threads", "query_time": datetime.now().isoformat()},
            )
    ...
    def get_aas_history(self, minutes: int = 30, interval: int = 60, use_persistent: bool = False) -> List[AASMetrics]:
        """
        获取AAS历史数据
    ...
    def identify_bottleneck(self) -> AASBottleneck:
        ...
        # 找出占比最高的类别
        sorted_categories = sorted(categories.items(), key=lambda x: x[1][1], reverse=True)
        ...
        # 生成描述和建议
        description = self._generate_bottleneck_description(primary, secondary, current)
    ...
    def _generate_bottleneck_description(
        ...
        """生成瓶颈描述"""
        cause_names = {"cpu": "CPU计算", "io": "IO等待", "lock": "锁等待", "network": "网络等待", "other": "其他等待"}
    ...
    def _generate_recommendations(self, bottleneck_type: str, metrics: AASMetrics) -> List[str]:
        ...
        recommendations = {
            ...
            "other": ["检查MySQL错误日志", "监控其他系统资源使用情况", "考虑升级MySQL版本", "联系DBA进行深度分析"],
        }

        return recommendations.get(bottleneck_type, recommendations["other"])

    def get_aas_trend_analysis(self, minutes: int = 60, use_persistent: bool = False) -> Dict[str, Any]:
        """
        获取AAS趋势分析
        ...
        返回:
            Dict: 趋势分析结果
        """
        history = self.get_aas_history(minutes=minutes, use_persistent=use_persistent)

        if len(history) < 2:
            return {"status": "insufficient_data", "message": f"数据点不足（需要至少2个，当前{len(history)}个）"}
        ...
        if std_aas > 0:
            for i, h in enumerate(history):
                ...
                if abs(z_score) > 2:
                    anomalies.append(
                        {"timestamp": h.timestamp.isoformat(), "value": h.total, "z_score": round(z_score, 2)}
                    )
        ...
        return {
            ...
            "anomalies": anomalies,
            "recommendations": self._generate_trend_recommendations(trend_direction, avg_aas, max_aas),
        }

    def _generate_trend_recommendations(self, trend: str, avg_aas: float, max_aas: float) -> List[str]:
        """生成趋势建议"""
        recommendations = []
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            "memory_history": {"data_points": memory_count, "max_capacity": self.config.max_history_size},
