...
class HostMappingManager:
    ...
    def auto_discover(self, prometheus_instances: List[str], zabbix_hosts: List[Dict]) -> List[HostInfo]:
        """
        自动发现主机映射
    ...
    def get_all_cluster_nodes(self, prometheus_name: str, zabbix_hosts: List[Dict]) -> List[Dict]:
        """
        获取 RDS 集群的所有节点
    ...
    def suggest_mapping(self, prometheus_name: str, zabbix_hosts: List[Dict]) -> List[Dict]:
        """
        ...
        for zbx_host in zabbix_hosts:
            ...
            if score > 0:
                suggestions.append(
                    {"hostid": zbx_host.get("hostid"), "name": zbx_name, "host": zbx_host_name, "score": score}
                )
