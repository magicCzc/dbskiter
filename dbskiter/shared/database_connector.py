...
class DatabaseConnector:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._pool_size = int(kwargs.get("pool_size", _os.getenv("DB_POOL_SIZE", self.POOL_SIZE)))
        self._pool_max_overflow = int(
            kwargs.get("pool_max_overflow", _os.getenv("DB_POOL_MAX_OVERFLOW", self.POOL_MAX_OVERFLOW))
        )
        self._pool_timeout = int(kwargs.get("pool_timeout", _os.getenv("DB_POOL_TIMEOUT", self.POOL_TIMEOUT)))
        self._pool_recycle = int(kwargs.get("pool_recycle", _os.getenv("DB_POOL_RECYCLE", self.POOL_RECYCLE)))
    ...
    def _build_connection_url(self) -> str:
        ...
        if self.dialect in ("sqlite", "sqlite3"):
            ...
            logger.info(f"方言 '{self.dialect}' 使用通用 JDBC URL 格式")
    ...
    def _build_generic_url(self) -> str:
        ...
        if self.host and self.port:
            return f"{self.dialect}://{self.username}:{encoded_password}" f"@{self.host}:{self.port}/{self.database}"
        elif self.database:
    ...
    def _build_mysql_url(self) -> str:
        ...
        encoded_password = quote_plus(self.password) if self.password else ""
        return f"mysql+pymysql://{self.username}:{encoded_password}" f"@{self.host}:{self.port}/{self.database}"

    def _build_postgresql_url(self) -> str:
        """构建 PostgreSQL URL"""
        encoded_password = quote_plus(self.password) if self.password else ""
        return f"postgresql://{self.username}:{encoded_password}" f"@{self.host}:{self.port}/{self.database}"

    def _build_sqlserver_url(self) -> str:
        """构建 SQL Server URL"""
        encoded_password = quote_plus(self.password) if self.password else ""
        return f"mssql+pyodbc://{self.username}:{encoded_password}" f"@{self.host}:{self.port}/{self.database}"
    ...
    def _build_oracle_url(self) -> str:
        ...
        if service_name:
            return f"oracle+{driver}://{self.username}:{encoded_password}" f"@{self.host}:{port}/{service_name}"
        else:
            return f"oracle+{driver}://{self.username}:{encoded_password}" f"@{self.host}:{port}"

    def _build_clickhouse_url(self) -> str:
        """构建 ClickHouse URL"""
        encoded_password = quote_plus(self.password) if self.password else ""
        return f"clickhouse+native://{self.username}:{encoded_password}" f"@{self.host}:{self.port}/{self.database}"
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"healthy": False, "error": str(e), "dialect": self.dialect}
    ...
    def get_tables(self) -> List[str]:
        ...
        sql = queries.get(
            self.dialect, "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE = 'BASE TABLE'"
        )
    ...
    def get_schema(self, table: str) -> pd.DataFrame:
        ...
        schema_queries = {
            "sqlite": (f"PRAGMA table_info({safe_table})", ["cid", "name", "type", "notnull", "dflt_value", "pk"]),
            "sqlite3": (f"PRAGMA table_info({safe_table})", ["cid", "name", "type", "notnull", "dflt_value", "pk"]),
            "mysql": (f"DESCRIBE `{safe_table}`", ["Field", "Type", "Null", "Key", "Default", "Extra"]),
            "mysql+pymysql": (f"DESCRIBE `{safe_table}`", ["Field", "Type", "Null", "Key", "Default", "Extra"]),
        }
