...
class ZabbixClient:
    ...
    def login(self, username: str, password: str) -> bool:
        ...
        # 先尝试新版参数
        result = self._call("user.login", {"username": username, "password": password})

        # 如果失败，尝试旧版参数
        if result["status"] != "success":
            result = self._call("user.login", {"user": username, "password": password})
    ...
    def get_items(self, host_id: str, key_search: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        查询主机的监控项
    ...
    def get_history(self, item_id: str, hours: int = 1, limit: int = 100) -> List[Dict[str, Any]]:
        """
        获取监控项的历史数据
...
class ZabbixMySQLMetrics:
    ...
    def get_disk_metrics(self, host_id: str) -> Dict[str, Any]:
        ...
        result = {"total_gb": None, "used_gb": None, "free_gb": None, "used_percent": None}
    ...
    def get_mysql_metrics(
        self, host_name: str, start_time: Optional[datetime] = None, end_time: Optional[datetime] = None
    ) -> Dict[str, Any]:
        """
        获取 MySQL 相关指标（兼容方法）
...
class ZabbixOracleMetrics:
    ...
    def get_group_metrics(self, group_name: str, hosts: List[Dict]) -> Dict[str, Any]:
        ...
        result = {"group": group_name, "hosts": {}, "aggregated": {}}
