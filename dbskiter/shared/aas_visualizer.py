...
class AASVisualizer:
    ...
    def generate_trend_chart(self, minutes: int = 60, metric: str = "total") -> str:
        """
        生成AAS趋势折线图（ASCII）
    ...
    def generate_stacked_chart(self, minutes: int = 60) -> str:
        ...
        lines = [f"AAS分类占比 (最近{minutes}分钟)", "=" * 50, ""]
    ...
    def generate_heatmap(self, hours: int = 24) -> str:
        ...
        lines.extend(["", "图例:  ░低  ▒中低  ▓中高  █高", f"最大AAS: {max_val:.2f}", "=" * 60])

        return "\n".join(lines)

    def generate_comparison_chart(self, periods: List[Tuple[str, int]] = None) -> str:
        """
        生成多时段对比图（ASCII柱状图）
        ...
        """
        if periods is None:
            periods = [("最近15分钟", 15), ("最近1小时", 60), ("最近6小时", 360), ("最近24小时", 1440)]
...
<body>
    ...
    def _extract_metric_values(self, history: List[Any], metric: str) -> List[float]:
        """提取指定指标的值列表"""
        metric_map = {
    ...
    def _create_line_chart(
        self, timestamps: List[datetime], values: List[float], title: str, y_label: str, x_label: str
    ) -> str:
        """创建ASCII折线图"""
        if not values:
            return "无数据"
    ...
    def _prepare_chart_data(self, history: List[Any]) -> Dict[str, Any]:
        """准备图表数据（JSON格式）"""
        if not history:
            return {"timestamps": [], "total": [], "cpu": [], "io": [], "lock": []}
