...
class OracleInspector(BaseInspector):
    ...
    # Oracle配置检查标准
    CONFIG_STANDARDS = {
        "processes": {"min": 150, "description": "最大进程数"},
        "sessions": {"min": 200, "description": "最大会话数"},
        "open_cursors": {"min": 300, "description": "最大打开游标数"},
        "db_cache_size": {"min": "100M", "description": "数据库缓存大小"},
        "shared_pool_size": {"min": "100M", "description": "共享池大小"},
        "pga_aggregate_target": {"min": "100M", "description": "PGA聚合目标"},
    }
    ...
    def inspect_configuration(self) -> List[InspectionItem]:
        ...
        for param_name, standard in self.CONFIG_STANDARDS.items():
            try:
                ...
                result = self._execute_query(f"SELECT value FROM v$parameter WHERE name = '{param_name}'")
            ...
            if hit_ratio < self.BUFFER_CACHE_HIT_RATIO_THRESHOLD:
                items.append(
                    self._create_item(
                        ...
                        risk_level=(
                            RiskLevel.HIGH if hit_ratio < self.BUFFER_CACHE_HIT_RATIO_CRITICAL else RiskLevel.MEDIUM
                        ),
                        status="warning",
