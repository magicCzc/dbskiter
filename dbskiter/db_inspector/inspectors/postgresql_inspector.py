...
class PostgreSQLInspector(BaseInspector):
    ...
    # PostgreSQL配置检查标准
    CONFIG_STANDARDS = {
        "max_connections": {"min": 100, "max": 1000, "description": "最大连接数"},
        "shared_buffers": {"min": "128MB", "recommend": "物理内存的25%", "description": "共享缓冲区大小"},
        "effective_cache_size": {"min": "1GB", "recommend": "物理内存的50-75%", "description": "有效缓存大小"},
        "work_mem": {"min": "4MB", "recommend": "根据并发连接数调整", "description": "工作内存"},
        "maintenance_work_mem": {"min": "64MB", "description": "维护工作内存"},
        "checkpoint_completion_target": {"min": 0.7, "max": 0.9, "description": "检查点完成目标"},
        "wal_buffers": {"min": "16MB", "description": "WAL缓冲区大小"},
        "default_statistics_target": {"min": 100, "description": "默认统计目标"},
        "random_page_cost": {"min": 1.0, "max": 4.0, "description": "随机页成本"},
        "effective_io_concurrency": {"min": 1, "description": "有效IO并发数"},
    }
    ...
    def inspect_configuration(self) -> List[InspectionItem]:
        ...
        for param_name, standard in self.CONFIG_STANDARDS.items():
            try:
                result = self._execute_query("SELECT setting, unit FROM pg_settings WHERE name = %s", (param_name,))
