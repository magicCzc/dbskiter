...
class MySQLInspector(BaseInspector):
    ...
    # MySQL配置检查标准 - 扩充到20+项
    CONFIG_STANDARDS = {
        "innodb_buffer_pool_size": {"min": "1G", "recommend": "物理内存的50-70%", "description": "InnoDB缓冲池大小"},
        "max_connections": {"min": 100, "max": 10000, "recommend": "根据业务需求设置", "description": "最大连接数"},
        "innodb_log_file_size": {"min": "48M", "recommend": "128M-1G", "description": "InnoDB日志文件大小"},
        "slow_query_log": {"expected": "ON", "description": "慢查询日志"},
        "long_query_time": {"max": 2, "recommend": "1-2秒", "description": "慢查询阈值"},
        "binlog_format": {"expected": "ROW", "description": "二进制日志格式"},
        ...
        "innodb_read_io_threads": {"min": 4, "recommend": "4-8", "description": "InnoDB读IO线程数"},
        "innodb_write_io_threads": {"min": 4, "recommend": "4-8", "description": "InnoDB写IO线程数"},
    }
    ...
    def inspect_configuration(self) -> List[InspectionItem]:
        ...
        for var_name, standard in self.CONFIG_STANDARDS.items():
            try:
                result = self._execute_query(f"SHOW VARIABLES LIKE '{var_name}'")
    ...
    def inspect_performance(self) -> List[InspectionItem]:
        ...
        try:
            ...
            if hit_rate < self.BUFFER_POOL_HIT_RATE_THRESHOLD:
                items.append(
                    self._create_item(
                        ...
                        risk_level=(
                            RiskLevel.HIGH if hit_rate < self.BUFFER_POOL_HIT_RATE_CRITICAL else RiskLevel.MEDIUM
                        ),
                        status="warning",
                ...
                if usage_rate > self.CONNECTION_USAGE_THRESHOLD:
                    items.append(
                        self._create_item(
                            ...
                            insp_type=InspectionType.PERFORMANCE,
                            risk_level=(
                                RiskLevel.HIGH if usage_rate > self.CONNECTION_USAGE_CRITICAL else RiskLevel.MEDIUM
                            ),
                            status="warning",
            ...
            if result:
                ...
                lock_ratio = (
                    (locks_waited / (locks_waited + locks_immediate)) * 100
                    if (locks_waited + locks_immediate) > 0
                    else 0
                )
