...
class CapacityMixin:
    ...
    def predict_capacity(self, metric: str, days: int = 30, source: str = "auto") -> Dict[str, Any]:
        """
        预测容量趋势
    ...
    def predict_capacity_advanced(self, metric: str, days: int = 30) -> Dict[str, Any]:
        """
        使用高级算法进行容量预测
        ...
        if not self.storage:
            return create_error_response("高级预测需要启用持久化存储", error_code=ErrorCode.STORAGE_ERROR)
        ...
        try:
            ...
            return create_error_response(
                "高级容量预测失败", error_code=ErrorCode.PREDICTION_FAILED, details={"error": str(e)}
            )
    ...
    def _predict_from_prometheus(self, metric: str, days: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # 准备数据并预测
            historical_data = [(datetime.fromisoformat(h["timestamp"]), h["value"]) for h in history]
            prediction = self.predictor.predict(metric, historical_data, days)

            return create_success_response(message="容量预测完成（数据来源：Prometheus）", data=prediction.to_dict())

        except Exception as e:
            logger.error(f"Prometheus 容量预测失败: {e}")
            return create_error_response(
                "Prometheus 容量预测失败", error_code=ErrorCode.PREDICTION_FAILED, details={"error": str(e)}
            )
        ...
        try:
            ...
            if is_oracle_group:
                ...
                if not matching_hosts:
                    return create_error_response(
                        f"在 Zabbix 中未找到资产组主机: {host_name}", error_code=ErrorCode.NOT_FOUND
                    )
                ...
                for host in matching_hosts:
                    ...
                    if items:
                        ...
                        if pused_items:
                            ...
                            logger.debug(
                                f"使用 pused 指标: {pused_items[0].get('name')} ({pused_items[0].get('key_')})"
                            )
                        else:
                ...
                if len(aggregated_history) >= 3:
                    ...
                    return create_success_response(
                        message="容量预测完成（数据来源：Zabbix Oracle资产组）", data=prediction_data
                    )
                else:
                    return create_success_response(
                ...
                items = self.zabbix_client.get_items(host["hostid"], key_search)
                if not items:
                    return create_error_response(f"未找到指标: {metric}", error_code=ErrorCode.NOT_FOUND)
                ...
                return create_success_response(message="容量预测完成（数据来源：Zabbix）", data=prediction.to_dict())

        except Exception as e:
            logger.error(f"Zabbix 容量预测失败: {e}")
            return create_error_response(
                "Zabbix 容量预测失败", error_code=ErrorCode.PREDICTION_FAILED, details={"error": str(e)}
            )
    ...
    def _predict_from_internal(self, metric: str, days: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(historical_data) >= 3:
                prediction = self.predictor.predict(metric, historical_data, days)
                return create_success_response(message="容量预测完成（数据来源：内部存储）", data=prediction.to_dict())
            ...
            if current_value is not None:
                ...
                prediction = CapacityPrediction(
                    ...
                    predictions={"current": current_value, "7d": current_value, "30d": current_value},
                    days_to_threshold=999,
                ...
                return create_success_response(
                    message="容量查询完成（仅当前值，无历史趋势）", data=prediction.to_dict()
                )
            ...
            return create_error_response(
                "容量预测失败", error_code=ErrorCode.PREDICTION_FAILED, details={"error": str(e)}
            )
