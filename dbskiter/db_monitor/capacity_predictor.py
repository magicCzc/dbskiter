...
class CapacityPredictor:
    ...
    def predict(
        ...
        days_to_threshold = self._calculate_days_to_threshold(current_value, adjusted_rate, threshold)

        # 确定紧急度
        urgency = self._determine_urgency(days_to_threshold)

        # 生成建议
        recommendation = self._generate_recommendation(metric, current_value, predictions, days_to_threshold, urgency)
    ...
    def predict_all_scenarios(
        self, metric: str, historical_data: List[Tuple[datetime, float]]
    ...
    def _clean_data(self, data: List[Tuple[datetime, float]]) -> List[Tuple[datetime, float]]:
        """清洗数据：排序、去重、去除异常值"""
        # 按时间排序
    ...
    def _calculate_trend(self, data: List[Tuple[datetime, float]]) -> Tuple[float, str]:
        """
        计算趋势
    ...
    def _adjust_rate(self, rate: float, scenario: PredictionScenario) -> float:
        """根据场景调整增长率"""
        adjustments = {
    ...
    def _predict_future(self, current_value: float, daily_rate: float) -> Dict[str, float]:
        """预测未来值"""
        return {
    ...
    def _calculate_days_to_threshold(self, current: float, daily_rate: float, threshold: float) -> Optional[int]:
        """计算达到阈值的天数"""
        if daily_rate <= 0:
    ...
    def _generate_recommendation(
        self, metric: str, current: float, predictions: Dict[str, float], days_to_threshold: Optional[int], urgency: str
    ) -> str:
        """生成容量规划建议"""
        if urgency == "critical":
...
def predict_capacity(
    metric: str, historical_data: List[Tuple[datetime, float]], scenario: str = "neutral"
) -> CapacityPrediction:
    """
    容量预测便捷函数
