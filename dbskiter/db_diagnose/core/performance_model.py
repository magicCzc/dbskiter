...
class PerformanceAnalyzer(ABC):
    ...
    def _execute_with_timeout(
        self, sql: str, params: Optional[Union[Tuple, Dict]] = None, timeout: Optional[int] = None
    ) -> Optional[List]:
        """
        带超时控制的SQL执行
    ...
    def analyze_bottleneck(self, snapshot: PerformanceSnapshot) -> List[Dict[str, Any]]:
        ...
        for category, metrics in category_metrics.items():
            high_severity = [m for m in metrics if m.get_severity() in (SeverityLevel.CRITICAL, SeverityLevel.HIGH)]
    ...
    def _get_suggestion(self, category: MetricCategory, metrics: List[PerformanceMetric]) -> str:
        """获取优化建议"""
        suggestions = {
