        ...
        try:
            with timer.step("select_engine", "选择数据库引擎适配"):
                if "mysql" in self.dialect:
                    ...
                    result = create_error_response(f"空间分析暂不支持 {self.dialect}", ErrorCode.UNSUPPORTED_SQL)
        ...
        try:
            ...
            for row in result.rows if result else []:
                ...
                if estimated_mb >= min_size_mb:
                    tables.append({"table": table_name, "size_mb": estimated_mb, "estimated": True, "rows": row_count})
            ...
            return create_success_response(
                message=f"获取到 {len(tables)} 个大表",
                data={
                    "total_space": {"total_mb": total_db_mb, "data_mb": round(total_data, 2), "free_mb": free_mb},
                    "large_tables": tables,
                    "suggestions": suggestions,
                },
            )
