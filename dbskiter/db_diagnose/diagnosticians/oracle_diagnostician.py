...
class OracleDiagnostician(BaseDiagnostician):
    ...
    def analyze_slow_queries(self, limit: int = 20, min_time: float = 1.0, use_awr: bool = True) -> Dict[str, Any]:
        """
        分析Oracle慢SQL（增强版）
        ...
        try:
            ...
            if use_awr:
                # 优先使用AWR历史数据
                report = analyzer.analyze_awr_history(hours=24, limit=limit, min_time=min_time)
            else:
                # 使用实时数据
                report = analyzer.analyze_realtime(limit=limit, min_time=min_time)

            # 转换为标准响应格式
            return self._create_result(
                success=True,
                message=f"成功分析 {report.total_queries} 个慢SQL，" f"发现 {report.unique_patterns} 个查询模式",
                data=report.to_dict(),
            )

        except Exception as e:
            logger.error(f"慢SQL分析失败: {e}")
            return self._create_result(success=False, message="慢SQL分析失败", error=str(e))
        ...
        try:
            ...
            if result:
                metrics["top_wait_events"] = [
                    {"event": row[0], "total_waits": row[1], "total_timeouts": row[2], "time_waited_sec": row[3]}
                    for row in result
                ]
    ...
    def get_database_stats(self) -> Dict[str, Any]:
        ...
        """
        try:
            stats = {"database_type": "Oracle", "timestamp": datetime.now().isoformat()}
    ...
    def analyze_index_usage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for row in result or []:
                hot_indexes.append({"schema": row[0], "table": row[1], "index": row[2], "usage_count": row[3]})
            ...
            for row in result or []:
                invalid_indexes.append({"schema": row[0], "table": row[1], "index": row[2], "status": row[3]})

            # 计算健康评分
            health_score = self._calculate_oracle_index_health_score(unused_indexes, invalid_indexes)
    ...
    def _calculate_oracle_index_health_score(self, unused_indexes: List[Dict], invalid_indexes: List[Dict]) -> int:
        """
        计算Oracle索引健康评分
    ...
    def analyze_tablespace_fragmentation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return self._create_result(success=False, message="表空间碎片分析失败", error=str(e))
