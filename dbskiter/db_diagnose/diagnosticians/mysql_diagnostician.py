...
class MySQLDiagnostician(BaseDiagnostician):
    ...
    def analyze_slow_queries(self, limit: int = 20, min_time: float = 1.0) -> Dict[str, Any]:
        ...
        try:
            ...
            slow_queries = self.slow_query_collector.collect_slow_queries(
                limit=limit, min_time=min_time, database=database
            )
            ...
            if not slow_queries:
                ...
                return self._create_result(
                    success=True, message=message, data={"total_queries": 0, "unique_patterns": 0, "queries": []}
                )
            ...
            for fp in fingerprints:
                if fingerprints[fp]["count"] > 0:
                    fingerprints[fp]["avg_time"] = fingerprints[fp]["total_time"] / fingerprints[fp]["count"]

            # 排序
            sorted_patterns = sorted(fingerprints.values(), key=lambda x: x["total_time"], reverse=True)
                ...
                queries_list.append(
                    {
                        ...
                        "timestamp": (
                            getattr(q, "timestamp", None).isoformat() if getattr(q, "timestamp", None) else None
                        ),
                    }
                )
            ...
            return self._create_result(success=False, message="数据库连接失败", error=str(e))
        except PermissionError as e:
            logger.error(f"权限不足: {e}")
            return self._create_result(success=False, message="权限不足", error=str(e))
        except ValueError as e:
            logger.error(f"数据解析错误: {e}")
            return self._create_result(success=False, message="数据解析错误", error=str(e))
    ...
    def _calculate_mysql_index_health_score(
        self, unused_indexes: List[Dict], tables_missing_index: List[Dict], redundant_indexes: List[Dict]
    ) -> int:
        """
        计算MySQL索引健康评分
    ...
    def analyze_table_fragmentation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return self._create_result(success=False, message="表碎片分析失败", error=str(e))
    ...
    def get_lock_waits(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return self._create_result(
                success=True, message=f"获取到 {len(waits)} 个锁等待", data={"lock_waits": waits}
            )
        except Exception as e:
    ...
    def analyze_performance_metrics(self, duration_minutes: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            if not samples:
                return self._create_result(success=False, message="未采集到AAS数据", error="采样失败")
            ...
            return self._create_result(success=False, message="采集超时", error=str(e))
    ...
    def get_database_stats(self) -> Dict[str, Any]:
        ...
        """
        try:
            stats = {"database_type": "MySQL", "timestamp": datetime.now().isoformat()}
            ...
            # 获取连接数
            result = self._execute_query("SELECT COUNT(*) FROM information_schema.processlist")
            if result:
                stats["current_connections"] = result[0][0]

            # 获取最大连接数
            result = self._execute_query("SHOW VARIABLES LIKE 'max_connections'")
            if result:
