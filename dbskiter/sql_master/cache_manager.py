...
class SQLCacheManager:
    ...
    def __init__(self, max_size: int = 1000, default_ttl: int = 300, enable_stats: bool = True):
        """
        初始化缓存管理器
        ...
        # 统计信息
        self._stats = {"hits": 0, "misses": 0, "sets": 0, "evictions": 0, "clears": 0}
    ...
    def set(self, sql: str, value: Any, params: Optional[Dict] = None, ttl: Optional[int] = None) -> bool:
        """
        设置缓存值
    ...
    def clear_expired(self) -> int:
        ...
        with self._lock:
            expired_keys = [key for key, (_, expire_time, _) in self._cache.items() if now > expire_time]
...
class CachedExecutor:
    ...
    def __init__(self, executor, cache_manager: Optional[SQLCacheManager] = None, enable_cache: bool = True):
        self.executor = executor
        self.cache = cache_manager or SQLCacheManager()
        self.enable_cache = enable_cache

    def execute(
        self, sql: str, params: Optional[Dict] = None, use_cache: bool = True, cache_ttl: Optional[int] = None
    ) -> Any:
        """
        执行SQL（带缓存）
