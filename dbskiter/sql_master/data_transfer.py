...
class DataExporter:
    ...
    def export_table(
        ...
        try:
            ...
            if not self._validate_identifier(table_name):
                return {"success": False, "message": f"表名包含非法字符: {table_name}", "exported_rows": 0}
            ...
            if columns:
                ...
                for col in columns:
                    if not self._validate_identifier(col):
                        return {"success": False, "message": f"列名包含非法字符: {col}", "exported_rows": 0}
            ...
            if where:
                if not self._validate_where_clause(where):
                    return {"success": False, "message": "WHERE子句包含危险内容，已拒绝执行", "exported_rows": 0}
                sql += f" WHERE {where}"
            ...
            if format_enum == DataFormat.CSV:
                ...
                return {"success": False, "message": f"不支持的导出格式: {format}", "exported_rows": 0}
            ...
            return {"success": False, "message": f"导出失败: {str(e)}", "exported_rows": 0}
    ...
    def _export_to_csv(self, rows: List[tuple], columns: List[str], output_path: str) -> None:
        """导出为CSV格式"""
    ...
    def _export_to_json(self, rows: List[tuple], columns: List[str], output_path: str) -> None:
        """导出为JSON格式"""
        data = []
        for row in rows:
            row_dict = {}
    ...
    def _export_to_sql(self, rows: List[tuple], columns: List[str], table_name: str, output_path: str) -> None:
        """导出为SQL INSERT语句"""
        with open(output_path, "w", encoding="utf-8") as f:
    ...
    def _export_to_excel(self, rows: List[tuple], columns: List[str], output_path: str) -> None:
        """导出为Excel格式"""
        try:
            import pandas as pd
        except ImportError:
    ...
    def import_csv(
        ...
        try:
            ...
            with open(input_path, "r", encoding=encoding) as f:
                ...
                if not columns:
                    return {"success": False, "message": "未指定列名且CSV没有表头", "imported_rows": 0}
            ...
            return {"success": False, "message": f"导入失败: {str(e)}", "imported_rows": 0}
    ...
    def import_json(self, input_path: str, table_name: str, batch_size: int = 1000) -> Dict[str, Any]:
        ...
        try:
            ...
            if not data or not isinstance(data, list):
                return {"success": False, "message": "JSON文件格式错误，应为对象数组", "imported_rows": 0}
