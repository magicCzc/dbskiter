...
class SchemaCache:
    ...
    def _load_mysql_schema(self):
        ...
        for row in tables_result.rows:
            ...
            table_info = TableInfo(
                name=table_name, comment=comment or "", row_count=row_count or 0, size_mb=size_mb or 0.0
            )
    ...
    def _load_indexes(self, table_info: TableInfo):
        ...
        table_info.indexes = [
            {"name": name, "columns": info["columns"], "unique": info["unique"]} for name, info in indexes.items()
        ]
    ...
    def _load_oracle_schema(self):
        ...
        for row in tables_result.rows:
            ...
            table_info = TableInfo(name=table_name, comment=comment, row_count=row_count, size_mb=size_mb)
            ...
            table_info.indexes = [
                {"name": name, "columns": info["columns"], "unique": info["unique"]}
    ...
    def search_columns(self, prefix: str, table_name: Optional[str] = None) -> List[Tuple[str, str]]:
        ...
        for match in matches:
            if (
                table_name
                and match != table_name
                and match not in self._tables.get(table_name, TableInfo(name="")).columns
            ):
                continue
...
class SQLIntelliSense:
    ...
    def get_suggestions(self, sql: str, cursor_pos: int, context: Optional[Dict] = None) -> List[SQLSuggestion]:
        """
        获取 SQL 补全建议
        ...
        if context_info["type"] == "table":
            ...
            for table in tables:
                suggestions.append(
                    SQLSuggestion(type="completion", text=table, description=f"表: {table}", confidence=1.0)
                )
            ...
            for table, col in columns[:10]:  # 最多10个
                ...
                suggestions.append(
                    SQLSuggestion(
                        type="completion", text=col, description=desc, confidence=0.9 if table == table_hint else 0.7
                    )
                )
            ...
            for keyword in self.SQL_KEYWORDS:
                if keyword.startswith(prefix):
                    suggestions.append(
                        SQLSuggestion(type="completion", text=keyword, description="SQL 关键字", confidence=0.8)
                    )
    ...
    def _parse_context(self, sql: str, cursor_pos: int) -> Dict[str, Any]:
        ...
        if re.search(r"\b(SELECT|WHERE|AND|OR|ORDER BY|GROUP BY)\s+.*$", sql_upper):
            ...
            return {"type": "column", "prefix": current_word, "table": tables[-1] if tables else None}
...
class SQLGuardian:
    ...
    def estimate_cost(self, sql: str, schema_cache: SchemaCache) -> Dict[str, Any]:
        ...
        返回：
            Dict: 成本估算结果
        """
        cost = {"estimated_rows": None, "estimated_time_ms": None, "warnings": [], "risk_level": "low"}
    ...
    def get_schema_info(self, table_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            if table_name:
                ...
                return {"status": "success", "success": True, "tables": tables, "count": len(tables)}
        except Exception as e:
            return {"status": "error", "success": False, "error": str(e)}
