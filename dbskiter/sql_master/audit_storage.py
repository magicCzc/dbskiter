...
class SQLAuditStorage:
    ...
    def get_statistics(self, days: int = 7) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            # 总记录数
            cursor = conn.execute("SELECT COUNT(*) FROM audit_records WHERE timestamp >= ?", (since,))
    ...
    def cleanup_old_records(self, days: int = 30) -> int:
        ...
        with self._lock:
            ...
            cursor = conn.execute("DELETE FROM audit_records WHERE timestamp < ?", (cutoff,))
            conn.commit()
    ...
    def get_record_by_id(self, record_id: int) -> Optional[SQLAuditRecord]:
        ...
        with self._lock:
            ...
            cursor = conn.execute("SELECT * FROM audit_records WHERE id = ?", (record_id,))
            row = cursor.fetchone()
