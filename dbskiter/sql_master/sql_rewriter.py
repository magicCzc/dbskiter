...
class SQLRewriter:
    ...
    def _apply_redundant_condition_to_sql(self, sql: str) -> str:
        ...
        for _ in range(max_iterations):
            ...
            rewritten = re.sub(r"\bWHERE\s+1\s*=\s*1\s+AND\s+", "WHERE ", rewritten, flags=re.IGNORECASE)

            # 情况2: ... AND 1=1 -> ... (1=1 在末尾或中间)
            rewritten = re.sub(r"\s+AND\s+1\s*=\s*1\b", "", rewritten, flags=re.IGNORECASE)

            # 情况3: 1=1 AND ... -> ... (1=1 在开头)
            rewritten = re.sub(r"\b1\s*=\s*1\s+AND\s+", "", rewritten, flags=re.IGNORECASE)

            # 情况4: WHERE 1=1 (后面没有其他条件，但可能有 ORDER/GROUP/LIMIT)
            rewritten = re.sub(
                r"\bWHERE\s+1\s*=\s*1\b(\s+(ORDER|GROUP|LIMIT|HAVING))", r"\1", rewritten, flags=re.IGNORECASE
            )

            # 情况5: WHERE 1=1 在末尾 -> 移除 WHERE 1=1
            rewritten = re.sub(r"\bWHERE\s+1\s*=\s*1\s*$", "", rewritten, flags=re.IGNORECASE)
    ...
    def _apply_select_star_to_sql(self, sql: str) -> str:
        ...
        rewritten = re.sub(
            r"SELECT(\s*/\*.*?\*/)?\s*\*", f"SELECT\\1 {column_str}", sql, flags=re.IGNORECASE | re.DOTALL
        )
    ...
    def _analyze_subquery(self, sql: str) -> Optional[RewriteSuggestion]:
        ...
        in_subquery_match = re.search(r"(\w+)\s+IN\s*\(\s*SELECT\s+([^)]+)\s+FROM\s+(\w+)", sql, re.IGNORECASE)
