...
class SchedulerEngine:
    ...
    def add_task(self, task_id: str, cron_expr: str, callback: Callable, **kwargs) -> bool:
        """添加任务"""
        with self._lock:
            self.tasks[task_id] = {"cron": cron_expr, "callback": callback, "kwargs": kwargs, "last_run": None}
    ...
    def get_task_list(self) -> List[Dict]:
        ...
        with self._lock:
            return [
                {"task_id": tid, "cron": task["cron"], "last_run": task["last_run"]} for tid, task in self.tasks.items()
            ]
