...
class BaseTaskExecutor(ABC):
    ...
    def _report_progress(
        self, callback: Optional[Callable[[ExecutionProgress], None]], phase: str, percent: float, message: str
    ):
        """报告进度"""
        if callback:
...
class BackupExecutor(BaseTaskExecutor):
    ...
    def _create_error_result(
        self, start_time: datetime, error: str, status: ExecutionStatus = ExecutionStatus.FAILED
    ) -> ExecutionResult:
        """创建错误结果"""
        return ExecutionResult(
            status=status, start_time=start_time, end_time=datetime.now(), message="执行失败", error=error
        )
...
class AnalyzeExecutor(BaseTaskExecutor):
    ...
    def execute(
        ...
        try:
            ...
            for i, table in enumerate(tables):
                ...
                self._report_progress(progress_callback, "分析", progress, f"分析表: {table} ({i+1}/{total_tables})")
    ...
    def _create_cancelled_result(self, start_time: datetime) -> ExecutionResult:
        return ExecutionResult(
            status=ExecutionStatus.CANCELLED, start_time=start_time, end_time=datetime.now(), message="任务已取消"
        )
        ...
        try:
            ...
            if tables:
                for i, table in enumerate(tables):
                    ...
                    self._report_progress(progress_callback, "清理", progress, f"清理表: {table}")
            ...
            return ExecutionResult(
                ...
                data={"tables": tables, "full": full, "analyzed": analyze_after},
                resource_usage=self._get_resource_usage(),
            )
...
class ReindexExecutor(BaseTaskExecutor):
        ...
        try:
            ...
            if indexes:
                ...
                for i, index in enumerate(indexes):
                    ...
                    self._report_progress(progress_callback, "重建", progress, f"重建索引: {index}")
                ...
                for i, table in enumerate(tables):
                    ...
                    self._report_progress(progress_callback, "重建", progress, f"重建表索引: {table}")
            ...
            return ExecutionResult(
                ...
                data={"rebuilt_indexes": rebuilt_indexes, "concurrently": concurrently},
                resource_usage=self._get_resource_usage(),
            )
    ...
    def _reindex(
        self, dialect: str, table: Optional[str] = None, index: Optional[str] = None, concurrently: bool = True
    ):
        """重建索引"""
        if "postgresql" in dialect:
    ...
    def execute(
        ...
        try:
            ...
            for i, table in enumerate(tables):
                ...
                progress = (i / len(tables)) * 100
                self._report_progress(progress_callback, "检查", progress, f"检查表: {table}")
            ...
            return ExecutionResult(
                ...
                data={"checked_tables": tables, "check_type": check_type, "results": check_results, "issues": issues},
                resource_usage=self._get_resource_usage(),
            )
    ...
    def execute(
        ...
        try:
            ...
            for i, sql in enumerate(sql_statements):
                ...
                self._report_progress(progress_callback, "执行", progress, f"执行SQL {i+1}/{len(sql_statements)}")
            ...
            return ExecutionResult(
                ...
                data={"statements": len(sql_statements), "results": results, "readonly": readonly},
