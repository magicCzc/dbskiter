...
def _add_output_mode_args(parser: argparse.ArgumentParser) -> None:
    ...
    需要像 --show-trace 一样注册到子解析器，
    使用户可以在子命令前后灵活使用 --output-mode。
    ...
    # 避免重复添加导致 argparse 冲突错误
    existing_dests = {a.dest for a in parser._actions if hasattr(a, "dest")}
    if "output_mode" in existing_dests:
        return
...
def _merge_global_args(parsed_args, raw_args: Optional[List[str]]) -> None:
    ...
    _add_output_mode_args(global_parser)
...
def add_subcommands(parser: argparse.ArgumentParser) -> None:
    ...
    for name, cmd_class in sorted(command_registry.items()):
        ...
        # 为子解析器也添加输出模式参数，支持 diagnose realtime --output-mode=ai 的写法
        _add_output_mode_args(subparser)
        # 让命令类添加自己的参数（可能会创建更深层的子解析器，如 diagnose -> slow-queries）
        cmd_class.add_arguments(subparser)
        # 递归为所有子子解析器也添加追踪参数和输出模式参数
        _add_trace_args_to_all_subparsers(subparser)
        _add_output_mode_args_to_all_subparsers(subparser)
...
def _add_output_mode_args_to_all_subparsers(parser: argparse.ArgumentParser) -> None:
    """
    递归为 parser 下的所有子解析器添加 --output-mode 参数。

    某些命令（如 diagnose、monitor）使用多层子解析器：
    dbskiter diagnose realtime --output-mode=ai
    这里的 --output-mode 需要被 realtime 子解析器识别。
    本函数遍历 parser 下的所有子解析器（包括孙解析器），为它们添加 --output-mode 参数。
...
def main(args: Optional[List[str]] = None) -> int:
    ...
    # 展开命令别名（扁平化入口）
    # 例如: "health" → ["monitor", "health"]
    expanded_args = expand_alias(raw_args)
    if expanded_args != raw_args:
        # 记录别名展开日志（调试用）
        pass  # 不输出，避免干扰命令结果

    # 处理特殊别名命令：welcome/hello/start/guide
    if expanded_args and expanded_args[0] in ("welcome", "hello", "start", "guide"):
        return _run_welcome_command(expanded_args)
    ...
    if args is None:
        args = sys.argv[1:]
    _merge_global_args(parsed_args, expanded_args)
    ...
    try:
        ...
        if needs_db:
            # 智能默认：如果未指定 --database，且只有一个配置，自动使用它
            _apply_smart_default(parsed_args)
...
def _run_welcome_command(args: List[str]) -> int:
    """
    运行 welcome / hello / start 新手引导命令
    ...
    console.print()
    console.print("[bold green]欢迎使用 DBSKiter！[/bold green]")
    console.print()
    console.print("[dim]数据库运维瑞士军刀 —— 监控、诊断、安全、调度、SQL 执行[/dim]")
    console.print()

    # 检测配置状态
    has_config = _check_has_config()
    has_shell = _check_has_shell_completion()

    if not has_config:
        console.print("[bold yellow]你还没有配置数据库[/bold yellow]")
        console.print()
        console.print("[bold]新手三步走：[/bold]")
        console.print()
        console.print("  [bold]1. 配置数据库[/bold]")
        console.print("     [cyan]dbskiter init[/cyan]")
        console.print("     → 交互式向导，2 分钟完成")
        console.print()
        console.print("  [bold]2. 健康检查[/bold]")
        console.print("     [cyan]dbskiter health[/cyan]")
        console.print("     → 查看数据库健康评分")
        console.print()
        console.print("  [bold]3. 生成巡检报告[/bold]")
        console.print("     [cyan]dbskiter report[/cyan]")
        console.print("     → 一键生成 HTML 报告")
        console.print()
        console.print("[dim]不想配置真实数据库？先体验演示模式：[/dim]")
        console.print("  [cyan]dbskiter --demo health[/cyan]")
        console.print()
    else:
        console.print("[bold green]已检测到数据库配置[/bold green]")
        console.print()
        console.print("[bold]常用命令：[/bold]")
        console.print("  [cyan]dbskiter health[/cyan]        - 健康检查")
        console.print("  [cyan]dbskiter top[/cyan]           - TOP SQL 分析")
        console.print("  [cyan]dbskiter slow[/cyan]          - 慢查询分析")
        console.print("  [cyan]dbskiter audit[/cyan]         - 安全审计")
        console.print("  [cyan]dbskiter report[/cyan]        - 生成巡检报告")
        console.print("  [cyan]dbskiter locks[/cyan]         - 锁分析")
        console.print()

    if not has_shell:
        console.print("[bold green]效率提升：[/bold green]")
        console.print("  [cyan]dbskiter shell-setup --auto[/cyan]")
        console.print("  → 启用 Tab 补全，命令不用背")
        console.print()

    console.print("[bold]更多帮助：[/bold]")
    console.print("  [cyan]dbskiter --help[/cyan]        - 查看所有命令")
    console.print("  [cyan]dbskiter init list[/cyan]     - 查看已配置的数据库")
    console.print()

    return 0


def _apply_smart_default(parsed_args: argparse.Namespace) -> None:
    """
    智能默认数据库

    当用户未指定 --database 时，如果只有一个数据库配置，
    自动使用该配置作为默认数据库。
