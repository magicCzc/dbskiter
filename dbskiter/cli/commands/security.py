  ...
  dbskiter --database=jump security high-risk --hours=48     # 检测48小时内高危操作
    ...
    def execute(self) -> int:
        ...
        try:
            ...
            if action == "audit":
                ...
                self.output.error(
                    "请指定安全操作: audit, sql-injection, sensitive-data, score, permissions, login-security, audit-log, high-risk, password-policy, weak-passwords, config"
                )
                return 1
            ...
            for module_name, module_result in modules.items():
                if isinstance(module_result, dict):
                    ...
                    if "success" in module_result and "data" in module_result:
                        ...
                        status_map = {"success": "完成", "failed": "失败", "completed": "完成", "error": "错误"}
    ...
    def _check_injection(self, skill) -> int:
        ...
        if findings:
            ...
            level = max((f.get("risk_level", "unknown") for f in findings), key=lambda x: level_priority.get(x, 0))
        else:
            level = "safe"
    ...
    def _scan_sensitive_data(self, skill) -> int:
        ...
        if findings:
            ...
            for i, finding in enumerate(findings[:10], 1):
                ...
                severity = (
                    finding.get("sensitivity_level") or finding.get("level") or finding.get("severity", "unknown")
                )
                ...
                if severity == "CRITICAL":
                    ...
                    self.output.warning(
                        f"  [{i}] [{severity}] {table}.{column}: {data_type} (置信度: {confidence:.0%})"
                    )
                else:
                    self.output.print(f"  [{i}] [{severity}] {table}.{column}: {data_type} (置信度: {confidence:.0%})")

                suggestion = (
                    finding.get("recommendation") or finding.get("suggestion") or finding.get("remediation", "")
                )
                if suggestion:
