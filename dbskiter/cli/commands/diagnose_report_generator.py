...
class DiagnoseReportGenerator:
    ...
    def _generate_executive_summary(
        self, snapshot_result: Dict[str, Any], bottleneck_result: Dict[str, Any]
    ) -> List[str]:
        """生成执行摘要（含健康评分）"""
        lines = ["## 执行摘要", ""]
    ...
    def _calculate_health_score(
        ...
        """计算健康评分"""
        # 默认满分
        scores = {"cpu": 100, "memory": 100, "io": 100, "concurrency": 100, "lock": 100}
    ...
    def _identify_risks(self, snapshot_result: Dict[str, Any], bottleneck_result: Dict[str, Any]) -> List[str]:
        """识别风险点"""
        risks = []
    ...
    def _generate_performance_snapshot(self, snapshot_result: Dict[str, Any]) -> List[str]:
        ...
        # 按类别显示
        categories = {"cpu": "CPU", "memory": "内存", "io": "IO", "concurrency": "并发", "lock": "锁"}
        ...
        for cat_key, cat_name in categories.items():
            ...
            for m in cat_metrics:
                ...
                unit = m.get("unit", "")
                severity = m.get("severity", "normal")
                status_text = {"critical": "严重", "high": "高危", "warning": "警告", "normal": "正常"}.get(
                    severity, "信息"
                )
    ...
    def _generate_slow_queries_section(self, snapshot_result: Dict[str, Any]) -> List[str]:
        ...
        for i, q in enumerate(slow_queries[:10], 1):
            sql_text = (
                q.get("sql_text", "")[:40] + "..." if len(q.get("sql_text", "")) > 40 else q.get("sql_text", "N/A")
            )
    ...
    def _generate_bottleneck_analysis(self, bottleneck_result: Dict[str, Any]) -> List[str]:
        ...
        if bottlenecks:
            ...
            for i, b in enumerate(bottlenecks[:5], 1):
                ...
                severity_text = {"critical": "致命", "high": "高危", "medium": "中危", "low": "低危"}.get(
                    severity, "信息"
                )
    ...
    def _generate_optimization_suggestions(
        self, bottleneck_result: Dict[str, Any], snapshot_result: Dict[str, Any]
    ) -> List[str]:
        """生成优化建议"""
        lines = ["## 优化建议", ""]
