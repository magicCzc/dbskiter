...
class SQLiteDDLAnalyzer(BaseDDLAnalyzer):
    ...
    def analyze_impact(self, ddl_sql: str) -> DDLImpact:
        ...
        # 评估执行时间
        impact.execution_time_estimate = self._estimate_sqlite_execution_time(operation, impact.table_size_mb)

        # 评估风险
        impact.risks = self._assess_sqlite_risks(operation, ddl_sql)

        # 生成建议
        impact.suggestions = self._generate_sqlite_suggestions(operation, ddl_sql, table_name)

        return impact

    def _estimate_sqlite_execution_time(self, operation: str, table_size_mb: float) -> str:
        """
        预估SQLite DDL执行时间
    ...
    def _assess_sqlite_risks(self, operation: str, ddl_sql: str) -> List[str]:
        ...
        if "DROP COLUMN" in sql_upper:
            risks.append("SQLite原生不支持DROP COLUMN，需要重建表")
            risks.append("重建表需要：创建新表 -> 复制数据 -> 删除旧表 -> 重命名")

        # 修改列类型需要重建表
        if "ALTER COLUMN" in sql_upper or "MODIFY" in sql_upper:
            risks.append("SQLite不支持直接修改列类型，需要重建表")

        # DROP TABLE风险
        if "DROP TABLE" in sql_upper:
            risks.append("DROP TABLE会永久删除表结构和数据，无法恢复")

        # 外键约束影响
        if "FOREIGN KEY" in sql_upper:
            risks.append("修改外键约束可能影响关联表的数据完整性")

        # 大表重建风险
        if "DROP COLUMN" in sql_upper or "MODIFY" in sql_upper:
            risks.append("重建表期间需要双倍磁盘空间（旧表+新表）")

        return risks

    def _generate_sqlite_suggestions(self, operation: str, ddl_sql: str, table_name: str) -> List[str]:
        """
        生成SQLite DDL建议
        ...
        if "DROP COLUMN" in sql_upper:
            suggestions.append("使用以下步骤安全删除列：")
            suggestions.append(f"1. BEGIN TRANSACTION;")
            suggestions.append(f"2. CREATE TABLE {table_name}_new AS SELECT ... FROM {table_name};")
            suggestions.append(f"3. DROP TABLE {table_name};")
            suggestions.append(f"4. ALTER TABLE {table_name}_new RENAME TO {table_name};")
            suggestions.append(f"5. COMMIT;")

        if "ALTER COLUMN" in sql_upper or "MODIFY" in sql_upper:
            suggestions.append("SQLite修改列需要重建表，建议：")
            suggestions.append("1. 创建新表（包含修改后的列定义）")
            suggestions.append("2. 复制数据（使用INSERT INTO ... SELECT）")
            suggestions.append("3. 删除旧表并重命名新表")

        if "ADD COLUMN" in sql_upper:
            suggestions.append("SQLite ADD COLUMN支持：NULL默认值、字面量默认值、CURRENT_TIMESTAMP")
            suggestions.append("不支持：UNIQUE约束、PRIMARY KEY、FOREIGN KEY（需重建表）")

        # 备份建议
        suggestions.append(f'执行前建议备份: sqlite3 {table_name}.db ".backup backup.db"')

        # 事务建议
        suggestions.append("建议在事务中执行DDL操作以保证原子性")

        # PRAGMA建议
        suggestions.append("执行前检查: PRAGMA foreign_keys; PRAGMA journal_mode;")
