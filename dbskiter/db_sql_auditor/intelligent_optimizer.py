...
class QueryRewriter:
    ...
    def _rewrite_select_star(self, sql: str, schema_info: Optional[Dict]) -> Dict[str, Any]:
        """重写SELECT *为具体列"""
        ...
        if schema_info and table_name in schema_info:
            ...
            if columns:
                ...
                optimized = re.sub(r"SELECT\s+\*", f"SELECT {column_list}", sql, flags=re.IGNORECASE)
    ...
    def _rewrite_implicit_conversion(self, sql: str, schema_info: Optional[Dict]) -> Dict[str, Any]:
        """消除隐式类型转换"""
        # 检测常见的隐式转换模式
        patterns = [
    ...
    def _rewrite_or_to_union(self, sql: str, schema_info: Optional[Dict]) -> Dict[str, Any]:
        """将OR条件重写为UNION"""
        # 检测OR条件
        ...
        if from_match and select_match:
            ...
            optimized = (
                f"{select_part} {from_part} WHERE {condition1} UNION ALL {select_part} {from_part} WHERE {condition2}"
            )
    ...
    def _rewrite_subquery(self, sql: str, schema_info: Optional[Dict]) -> Dict[str, Any]:
        """优化子查询"""
    ...
    def _eliminate_redundant_conditions(self, sql: str, schema_info: Optional[Dict]) -> Dict[str, Any]:
        """消除冗余条件"""
        # 检测1=1这种冗余条件
        redundant_patterns = [
...
class IndexRecommender:
    ...
    def detect_redundant_indexes(self, existing_indexes: List[Dict]) -> List[Dict[str, Any]]:
        """
        检测冗余索引
    ...
    def _extract_conditions(self, sql: str) -> List[Dict[str, Any]]:
        ...
        if where_match:
            ...
            if columns:
                conditions.append({"table": table, "columns": list(set(columns)), "type": "where"})  # 去重

        # 提取JOIN条件
        join_matches = re.findall(
            r"JOIN\s+(\w+)\s+ON\s+(.+?)(?:JOIN|WHERE|ORDER|GROUP|LIMIT|$)", sql, re.IGNORECASE | re.DOTALL
        )
        for table, on_clause in join_matches:
            ...
            if columns:
                conditions.append({"table": table, "columns": list(set(columns)), "type": "join"})

        return conditions

    def _has_index(self, existing_indexes: Optional[List[Dict]], table: str, columns: List[str]) -> bool:
        """检查是否已有索引"""
...
class CostEstimator:
    ...
    def estimate(self, sql: str, table_stats: Dict[str, Any]) -> CostEstimate:
        """
        估算SQL执行成本
    ...
    def compare_costs(self, original_cost: CostEstimate, optimized_cost: CostEstimate) -> Dict[str, Any]:
        """
        对比优化前后的成本
    ...
    def _estimate_io_cost(self, sql: str, table_stats: Dict[str, Any], sql_type: str) -> float:
        """估算IO成本"""
        # 简化模型：基于表大小和查询类型
...
class IntelligentOptimizer:
    ...
    def optimize(
        ...
        # 2. 索引推荐
        if schema_info:
            index_recs = self.index_recommender.recommend_indexes(sql, schema_info, existing_indexes)
        ...
        if table_stats:
            ...
            if rewrite_result.get("optimized_sql"):
                optimized_cost = self.cost_estimator.estimate(rewrite_result["optimized_sql"], table_stats)
                cost_comparison = self.cost_estimator.compare_costs(original_cost, optimized_cost)
                result["cost_comparison"] = cost_comparison

        # 按优先级排序建议
        priority_order = {"CRITICAL": 0, "HIGH": 1, "MEDIUM": 2, "LOW": 3}
        result["recommendations"].sort(key=lambda x: priority_order.get(x["priority"], 4))
