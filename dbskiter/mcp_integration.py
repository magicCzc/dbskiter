...
class MCPSecurityWrapper:
    ...
    def __init__(self, connector: UnifiedConnector, enable_audit: bool = True):
        """
        初始化MCP安全包装器
        ...
        self.security_executor = SecurityExecutor(connector=connector)
    ...
    def execute_sql(
        ...
        if is_readonly_mode():
            ...
            if not allowed:
                ...
                return {"success": False, "error": reason, "risk_level": "HIGH", "blocked_by": "readonly_middleware"}
        ...
        try:
            ...
            return {"success": False, "error": f"SQL解析失败: {str(e)}"}

        # 3. 使用安全执行器执行
        result = self.security_executor.execute(
            sql=sql, database=database, force=force, confirmed=confirmed, user=user, client_ip=client_ip
        )
    ...
    def validate_sql(self, sql: str, database: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"valid": False, "error": f"SQL解析失败: {str(e)}"}
...
class SecureSQLTool:
    ...
    def execute(self, database: str, sql: str, force: bool = False, confirmed: bool = False) -> Dict[str, Any]:
        """
        执行SQL（MCP工具标准接口）
        ...
        返回：
            Dict: 执行结果
        """
        return self.wrapper.execute_sql(sql=sql, database=database, force=force, confirmed=confirmed)

    def validate(self, database: str, sql: str) -> Dict[str, Any]:
        """
        验证SQL（MCP工具标准接口）
...
def execute_sql_safely(
    sql: str, database: str, connector: UnifiedConnector, force: bool = False, confirmed: bool = False
) -> Dict[str, Any]:
    """
    安全执行SQL的便捷函数
    ...
    with MCPSecurityWrapper(connector) as wrapper:
        return wrapper.execute_sql(sql=sql, database=database, force=force, confirmed=confirmed)
