...
class SQLInjectionDetector:
    ...
    def analyze_sql(self, sql: str, params: Optional[Dict] = None, dialect: str = "mysql") -> Dict[str, Any]:
        """
        深度分析 SQL 注入风险
        ...
        has_injection = any(f.risk_level in (RiskLevel.CRITICAL, RiskLevel.HIGH) for f in unique_findings)
    ...
    def detect(self, sql: str, params: Optional[Dict] = None, dialect: str = "mysql") -> Dict[str, Any]:
        """
    ...
    def _has_user_input_trace(self, clause: str) -> bool:
        ...
        if re.search(r"['\"]\s*\bOR\b", clause, re.IGNORECASE) or re.search(r"['\"]\s*\bAND\b", clause, re.IGNORECASE):
            return True
    ...
    def _detect_comment_injection(
        self, where_clause: str, sql_hash: str, sql_preview: str
    ) -> List[SQLInjectionFinding]:
        """检测注释注入"""
        findings = []
    ...
    def _detect_union_injection(
        self, statement: Statement, sql_hash: str, sql_preview: str
    ) -> List[SQLInjectionFinding]:
        """检测 UNION 注入
    ...
    def _calculate_risk_score(self, findings: List[SQLInjectionFinding]) -> float:
        ...
        weights = {RiskLevel.CRITICAL: 100, RiskLevel.HIGH: 50, RiskLevel.MEDIUM: 20, RiskLevel.LOW: 5}
